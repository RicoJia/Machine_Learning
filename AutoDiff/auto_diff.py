...
class Tensor:
    ...
    def __mul__(self, other):
        ...
        out = self._binary_forward_operation(
                    other=other, 
                    binary_op=lambda this_data,other_data: this_data * other_data)
        def backward_func():
            if self._required_grad:
...
class MSELoss:
    ...
    def __call__(self, output: Tensor, target: Tensor):
        ...
        def backward_func():
            self._grad = 1.0
            self._model_output._grad = -2.0 / n * (target._data - self._model_output._data)
...
if __name__ == "__main__":
    ...
    test_info_ls = [
        TestInfo( k = 0, w1_val= 1,),
        TestInfo(k = -1, w1_val= 1,),
    ...
    for t in test_info_ls:
        test_y_squared_minus_first_order(w1_val=t.w1_val, k=t.k, expected_grad=true_grad(t))
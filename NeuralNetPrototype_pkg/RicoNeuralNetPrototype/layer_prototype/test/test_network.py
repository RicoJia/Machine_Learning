...
from RicoNeuralNetPrototype.layer_prototype.rico_nn import (
    LR,
    RicoNNBase,
    one_step_rico,
    one_step_torch,
...
def assign_weights_to_model(model, torch_model):
    ...
    for layer, torch_layer in zip(rico_calculation_layers, torch_calculation_layers):
        # shape is torch.Size
...
def check_layer_outputs(torch_nn, rico_nn):
    ...
    for layer_num, (rico_layer_output, torch_layer_output) in enumerate(
        zip(rico_nn.layer_outputs, torch_nn.layer_outputs)
    ):
        # TODO Remember to remove
        print(
            f"Layer num: {layer_num}. layer output: {rico_layer_output.shape}, torch layer output: {torch_layer_output.shape}"
        )
        # TODO Remember to remove
        ...
        assert np.allclose(
            rico_layer_output, torch_layer_output.detach().numpy(), rtol=1e-3, atol=1e-2
        )
...
def test_network():
    ...
    check_layer_outputs(torch_nn=torch_nn, rico_nn=rico_nn)

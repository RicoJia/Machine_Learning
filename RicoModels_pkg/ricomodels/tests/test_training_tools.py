...
def test_validate_model():
    ...
    labels = torch.randint(0, output_size, (100,))  # 100 random labels in [0, output_size)
...
def test_early_stopping():
    ...
    for i in range(1000):
        ...
        if early_stopping(0.4):
            break
    assert counter == patience, f"Please check early stopping logic. Counter value {counter} should be equal to patience {patience}"
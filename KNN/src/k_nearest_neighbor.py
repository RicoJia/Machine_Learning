...
class KNearestNeighbor():    
    ...
    def predict(self, features, ignore_first=False):
        ...
        for row in dist_matrix:
            ...
            total_matrix = np.hstack((self.model_features, self.model_targets, row.reshape(row.shape[0], 1)))
            ...
            #get k neighbors
            k_neighbors = total_matrix[:self.n_neighbors, feature_cln: feature_cln+targets_cln]
            k_neighbors_ls = k_neighbors.tolist()

            #make prediction
            if self.aggregator=='mode':      #Assumption: all these functions are applied along each column
                prediction = np.apply_along_axis(lambda x: max(set(x.tolist()), key=(x.tolist()).count), axis=0, arr=k_neighbors)

...
def test_gmm_spec():
    features, targets = generate_cluster_data(
        n_samples=100, n_features=2, n_centers=2, cluster_stds=0.1
    )
    gmm = GMM(2, "spherical")
    gmm.fit(features)
...
def test_gmm_likelihood():
    ...
    for k in range(means.shape[0]):
        scipy_prob = multivariate_normal.logpdf(features, means[k], covariances[k])
        ...
        for f in n_features:
            for c in n_centers:
                for s in stds:
                    ...
                    actual_mixing_weights = (
                        actual_mixing_weights / actual_mixing_weights.sum()
                    )

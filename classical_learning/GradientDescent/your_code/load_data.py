...
def load_data(dataset, fraction=1.0, base_folder="data"):
    ...
    if dataset == "blobs":
        ...
        train_features, test_features, train_targets, test_targets = load_json_data(
            path
        )
    elif dataset == "mnist-binary":
        train_features, test_features, train_targets, test_targets = load_mnist_data(
            2, fraction=fraction, mnist_folder=base_folder
        )
        train_targets = train_targets * 2 - 1
        test_targets = test_targets * 2 - 1
    elif dataset == "mnist-multiclass":
        train_features, test_features, train_targets, test_targets = load_mnist_data(
            5, fraction=fraction, examples_per_class=100, mnist_folder=base_folder
        )

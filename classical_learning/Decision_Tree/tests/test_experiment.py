...
datasets = [os.path.join("data", x) for x in os.listdir("data") if ".csv" in x]
...
def test_experiment_run_decision_tree():
    ...
    accuracy_goals = {
        xp_dataset_name("ivy-league.csv"): 0.95,
        xp_dataset_name("xor.csv"): 1.0,
        xp_dataset_name("candy-data.csv"): 0.75,

...
def confusion_matrix(actual, predictions):
    ...
    true_positives = len([i for i in range(len(actual)) if (actual[i]==predictions[i]) and (actual[i]>0) ])
    true_negatives = true_total - true_positives

    false_total = len(actual) - true_total
    false_positives = len([i for i in range(len(actual)) if (actual[i]!=predictions[i])and(predictions[i]>0 ) ])

...
class FCNDebugger:
    def __init__(self, model: torch.nn.Module, config: FCNDebuggerConfig):
        self.model = model
        self.config = config
        self._losses = []
        self._gradient_norms = defaultdict(list)
        self._initial_weights = self._save_weights()
        self._plot_func_ids: list = [(self._plot_costs, 'cost-plot'), 
                               (self._plot_grad_norms, 'grad-norm-plot'),
                               (self._plot_weight_statistics, 'weights-distribution')]
    ...
    def _calculate_weight_statistics(self):
        ...
        for layer_name, weights in self._initial_weights.items():
            mean_variances[layer_name] = {
                ...
                'variance': sum((x - sum(weights) / len(weights)) ** 2 for x in weights) / len(weights)
            }
        return mean_variances

    def _format_fig(self, fig, title, xaxis_title, yaxis_title):
        fig.update_layout(
            title=title,
            ...
            plot_bgcolor='rgba(245, 245, 220, 1.0)',   # Transparent plot area
            # plot_bgcolor='rgba(0, 0, 0, 0)',   # Transparent plot area
            showlegend=False,
        )

    def _plot_weight_statistics(self):
        # Create a box plot for each layer's initial weights
        fig = go.Figure()
        for layer_name, weights in self._initial_weights.items():
            fig.add_trace(go.Box(y=weights, name=layer_name, boxmean='sd', 
    ...
    def plot_summary(self):
        ...
        for i in range(0, len(self._plot_func_ids), 2):
            ...
            children = [get_div(func=self._plot_func_ids[i][0], id = self._plot_func_ids[i][1])]
            if i+1 < len(self._plot_func_ids):
                children.append(
                    get_div(func=self._plot_func_ids[i+1][0], id = self._plot_func_ids[i+1][1],)
                )
            divs.append(html.Div(children=children))
                
        # Layout of the app
        app.layout = html.Div(children=[
            html.H1(children='Training Summary', style={
                'font-family': 'Arial',
                'font-size': '36px',
                'color': 'black',
                'text-align': 'center'    
...
class FCN2DDebugger(FCNDebugger):
    def __init__(self, model: Module, config: FCNDebuggerConfig, X_test, y_test):
        super().__init__(model, config)
        self._plot_func_ids.append(
            (self._plot_decision_boundary, "Decision Boundary")
        )
    ...
    def _plot_decision_boundary(self, resolution=0.02):
        ...
        cmap = [colors[i] for i in np.unique(self._y_test).astype(int)]
        ...
        Z = self.model(torch.from_numpy(np.array([xx1.ravel(), xx2.ravel()]).astype(np.float32).T))
        ...
        scatter_data = []
        for idx, cl in enumerate(np.unique(self._y_test)):
            scatter = go.Scatter(
                x=self._X_test[np.reshape(self._y_test == cl, (-1,)), 0],
                y=self._X_test[np.reshape(self._y_test == cl, (-1,)), 1],
                mode='markers',
                name=f'Class {cl}',
                marker=dict(
                    color=colors[idx],
        ...
        self._format_fig(fig=fig, title='Decision Boundary', xaxis_title='Feature 1', yaxis_title='Feature 2')
        return fig
